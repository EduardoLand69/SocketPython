...
with sk.socket(sk.AF_INET, sk.SOCK_STREAM) as cliente:
    ...
    if msg_servidor.decode('utf-8') == "ok": #el servidor tiene el directorio
        ...
        if msg_servidor.decode('utf-8') == "no":
            ...
            if respuesta_cliente == "y" or respuesta_cliente == "Y":
                cliente.sendall(respuesta_cliente.encode('utf-8'))
                #el servidor esta pidiendo el nombre del archivo
                servidorPideNombreArchivo = cliente.recv(1024)
                respuesta_c = input(servidorPideNombreArchivo.decode('utf-8'))
                
                #el servidor pregunta si se quiere escribir en el archivo
                #mandamos el nombre del archivo
                cliente.sendall(respuesta_c.encode('utf-8'))
            if respuesta_cliente == "n" or respuesta_cliente == "N":
    ...
    if msg_servidor.decode('utf-8') == "no": #el servidor no tiene el directorio
        #bloque de codigo que se ejeuctara si no existe el directorio
        #recibir pregunta del servidor
        pregunta= cliente.recv(1024) #"Quieres crear el directorio?
        respuesta_cliente = input(pregunta.decode('utf-8'))
        if respuesta_cliente == "y" or respuesta_cliente == "Y":
            cliente.sendall(respuesta_cliente.encode('utf-8'))
            ...
            respuesta_c = input(pregunta.decode('utf-8')) #pregunta si se quiere crear un archivo nuevo
            if respuesta_c == 'y' or respuesta_c =='Y':
                cliente.sendall(respuesta_cliente.encode('utf-8'))
                print("Se creo el archivo en el servidor")
                #el servidor esta pidiendo el nombre del archivo
                servidorPideNombreArchivo = cliente.recv(1024)
                respuesta_c = input(servidorPideNombreArchivo.decode('utf-8'))
            ...
            if respuesta_c == "n" or respuesta_c == "N":
                print("No se creo el archivo :(")
                tm.sleep(3)
                exit
